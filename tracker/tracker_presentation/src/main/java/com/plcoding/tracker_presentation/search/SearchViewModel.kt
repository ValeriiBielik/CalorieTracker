...
class SearchViewModel @Inject constructor(
    ...
    fun onEvent(event: SearchEvent) {
        when (event) {
            is SearchEvent.OnAmountForFoodChange -> state = state.copy(trackableFood = state.trackableFood.map {
            ...
            is SearchEvent.OnSearchFocusChange -> state = state.copy(isHintVisible = !event.isFocused && state.query.isBlank())
            is SearchEvent.OnToggleTrackableFood -> state = state.copy(trackableFood = state.trackableFood.map {
                if (it.food == event.food) {
